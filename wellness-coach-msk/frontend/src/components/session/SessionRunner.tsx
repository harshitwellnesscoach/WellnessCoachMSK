...
export default function SessionRunner({ sessionId, exercises }: Props) {
  ...
  // Pose tracking rAF loop — runs during EXERCISES and RESTING
  useEffect(() => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      {/* Camera — kept mounted across EXERCISES/RESTING so permission isn't re-requested */}
      <div style={{ display: showCamera ? 'block' : 'none' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
          {cameraError
            ? <span className="alert alert-warn" style={{ fontSize: 13, padding: '4px 10px' }}>{cameraError}</span>
            : <span />
          }
          {fps > 0 && (
            <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>{fps} fps</span>
          )}
        </div>
      </div>
      ...
      {phase === 'EXERCISES' && exercise && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            {exercise.hold_seconds ? (
              <span style={{
                ...
                borderRadius: 20, border: '1px solid var(--purple-border)',
              }}>
          ...
          <div style={{
            background: 'var(--purple-light)', border: '1.5px solid var(--purple-border)',
      ...
      {phase === 'RESTING' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16, textAlign: 'center' }}>
          ...
          <button className="btn btn-outline" style={{ width: '100%' }} onClick={() => setRestLeft(0)}>
            Skip Rest
          </button>
        </div>
      )}
      ...
      {phase === 'COMPLETE' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24, textAlign: 'center' }}>
          <div style={{
            ...
            fontSize: 28, fontWeight: 900, color: '#fff', margin: '0 auto',
          }}>✓</div>
          <div>
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            <div style={{
              ...
              border: '1.5px solid var(--purple-border)', borderRadius: 12, padding: 16,
            }}>
